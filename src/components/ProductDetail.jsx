...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      try {
        ...
        // Set meta info from product data
        if (productData.meta_title || productData.meta_description) {
          setMetaInfo({
            ...
            canonical: productData.meta_canonical
          });
        }
      } catch (error) {
        console.error('Error loading product:', error);
  ...
  const getMetaInfo = () => {
    if (metaInfo && metaInfo.title) {
      return {
        ...
        description: metaInfo.description || product?.shortDescription || 'Gastronova pharmaceutical product',
        canonical: metaInfo.canonical || `https://gastronova.in/products/${slug}`,
        image: product?.images?.[0] || 'https://admin-gastronova.webhostdevs.com/uploads/medications/default-product.jpg'
      };
    }

    // Fallback meta info
    return {
      title: `${product?.name || 'Product'} | Gastronova Pharmaceuticals`,
      description: product?.shortDescription || 'High-quality pharmaceutical product from Gastronova',
      canonical: `https://gastronova.in/products/${slug}`,
      image: product?.images?.[0] || 'https://admin-gastronova.webhostdevs.com/uploads/medications/default-product.jpg'
    };
  };
  ...
  return (
    <>
      {/* COMPREHENSIVE HELMET FOR PRODUCT DETAIL SEO */}
      <Helmet>
        <title>{meta.title}</title>
        <meta name="description" content={meta.description} />
        <meta name="keywords" content={`${product?.name}, pharmaceutical, medicine, healthcare, Gastronova, GI, liver care`} />
        ...
        <meta property="og:site_name" content="Gastronova Pharmaceuticals" />
        ...
        <meta name="twitter:site" content="@gastronova" />
        ...
        <meta name="author" content="Gastronova Pharmaceuticals" />
      </Helmet>
